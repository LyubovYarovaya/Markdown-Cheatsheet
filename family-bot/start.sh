...
# Перезапускаем при падении: разрыв сети или случайная ошибка не должны
# оставлять бота молчащим до утра.
while true; do
  set +e
  "$VENV_PY" -m app.main
  code=$?
  ...
  warn "Приложение остановилось (код $code). Перезапускаю через 3 секунды…"
  sleep 3
done
